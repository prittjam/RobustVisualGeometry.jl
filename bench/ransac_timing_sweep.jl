#!/usr/bin/env julia
# =============================================================================
# Benchmark: RANSAC timing vs. noise level and outlier rate
#
# Sweeps σ × outlier_rate for homography and fundamental matrix estimation.
# Reports median time, inlier count, ŝ/σ ratio, and reproj/Sampson error.
#
# Three RANSAC variants:
#   A. No LO            (MarginalScoring{Nothing},    NoLocalOptimization)
#   B. LO model-certain (MarginalScoring{Nothing},    PosteriorIrls)
#   C. LO predictive    (PredictiveMarginalScoring,   PosteriorIrls)
#
# Usage:
#   julia --project=. bench/ransac_timing_sweep.jl
#   julia --project=. bench/ransac_timing_sweep.jl --homography
#   julia --project=. bench/ransac_timing_sweep.jl --fundmat
# =============================================================================

using LinearAlgebra, StaticArrays, Random, Printf, Statistics
using BenchmarkTools
using RobustVisualGeometry
using VisualGeometryCore: PerspectiveMap, ProjectiveMap, HomographyMat, FundMat,
                          sampson_distance
...
struct BenchResult
    ...
    inliers::Int
    scale::Float64
    error::Float64     # reproj for H, max Sampson for F
    quality::Float64
    trials::Int
end
...
function run_sweep(bench_fn, make_data_fn, problem_label;
                   σ_range, outlier_fracs, n_inliers, a,
                   max_trials, n_reps)
    ...
    println()
    println("=" ^ W)
    println("  $problem_label — timing + accuracy sweep (BenchmarkTools @belapsed)")
        ...
        for σ in σ_range
            for out_frac in outlier_fracs
                ...
                for rep in 1:n_reps
                    ...
                    cs = make_data_fn(rng, σ, n_inliers, n_outliers)
                    shuffle!(rng, cs)
                    r = bench_fn(cs, a, config, lo, scoring_ctor)
...
function main()
    run_h = isempty(ARGS) || "--homography" in ARGS
    run_f = isempty(ARGS) || "--fundmat" in ARGS
    ...
    if run_h
        run_sweep(bench_homography, make_homography_data, "Homography";
                  σ_range, outlier_fracs, n_inliers=200, a=500.0,
                  max_trials=5000, n_reps)
    end

    if run_f
        run_sweep(bench_fundmat, make_fundmat_data, "Fundamental Matrix";
                  σ_range, outlier_fracs, n_inliers=200, a=500.0,
                  max_trials=50_000, n_reps)
    end
end
